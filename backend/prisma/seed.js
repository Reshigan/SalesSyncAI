...
async function main() {
    ...
    const adminPassword = await bcryptjs_1.default.hash('TestAdmin123!', 12);
    ...
    const regionalManagerPassword = await bcryptjs_1.default.hash('RegionalManager123!', 12);
    const regionalManager = await prisma.user.create({
        data: {
            ...
            lastName: 'Regional',
            phone: '+27123456791',
            role: client_1.UserRole.REGIONAL_MANAGER,
            permissions: [
                'agents.manage',
                'visits.view',
                'reports.view',
    ...
    const salesAgentPassword = await bcryptjs_1.default.hash('SalesAgent123!', 12);
    const salesAgent = await prisma.user.create({
        data: {
            ...
            phone: '+27123456792',
            role: client_1.UserRole.FIELD_SALES_AGENT,
            permissions: [
                'visits.create',
                'sales.create',
                'stock.draw',
    ...
    const marketingAgentPassword = await bcryptjs_1.default.hash('MarketingAgent123!', 12);
    const marketingAgent = await prisma.user.create({
        data: {
            ...
            lastName: 'Marketer',
            phone: '+27123456793',
            role: client_1.UserRole.FIELD_MARKETING_AGENT,
            permissions: [
                'campaigns.execute',
                'surveys.create',
                'customers.create',
    ...
    // Create Promoter
    const promoterPassword = await bcryptjs_1.default.hash('Promoter123!', 12);
    const promoter = await prisma.user.create({
        data: {
            ...
            lastName: 'Promoter',
            phone: '+27123456794',
            role: client_1.UserRole.PROMOTER,
            permissions: [
                'activations.execute',
                'events.manage',
                'content.create'
    ...
    for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
        ...
        for (let v = 0; v < visitsPerDay; v++) {
            ...
            visits.push({
                ...
                gpsLocation: {
                    arrival: {
                        lat: customer.coordinates.lat + (Math.random() - 0.5) * 0.001,
                        lng: customer.coordinates.lng + (Math.random() - 0.5) * 0.001
                    },
                    departure: {
                        lat: customer.coordinates.lat + (Math.random() - 0.5) * 0.001,
                        lng: customer.coordinates.lng + (Math.random() - 0.5) * 0.001
    ...
    for (const visit of allVisits) {
        ...
        if (Math.random() < 0.7) {
            ...
            const paymentMethod = Math.random() < 0.8 ? client_1.PaymentMethod.CASH : client_1.PaymentMethod.CARD;
            sales.push({
                companyId: testCompany.id,
        ...
        data: [
            {
                ...
                type: client_1.CampaignType.BRAND_ACTIVATION,
            ...
            },
            {
                ...
                type: client_1.CampaignType.PRODUCT_LAUNCH,
