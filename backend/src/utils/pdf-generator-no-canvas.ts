/**
 * Canvas-free PDF Generation Utility for SalesSync
 * Generates professional reports without native dependencies
 */
...
export class PDFGeneratorNoCanvas {
  ...
  constructor(options: PDFOptions) {
    ...
    // Initialize QuickChart for external chart generation
    this.quickChart = new QuickChart();
    this.quickChart.setWidth(800);
  ...
  private async generatePerformanceReport(data: any): Promise<void> {
    ...
    // Territory Performance Chart using QuickChart
  ...
  /**
   * Add chart using QuickChart API (no canvas required)
   */
  private async addQuickChart(config: ChartConfig): Promise<void> {
    try {
      ...
      // Get chart URL instead of rendering locally
      const chartUrl = this.quickChart.getUrl();
      
      this.checkPageBreak(300);
      
      // Add chart URL as text (or you could fetch the image and embed it)
      this.addText(`Chart: ${chartUrl}`);
      
      // Alternative: Fetch and embed the chart image
 ...
 */
export async function generatePDFNoCanvas(options: PDFOptions): Promise<Buffer> {
  const generator = new PDFGeneratorNoCanvas(options);
