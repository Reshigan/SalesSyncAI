...
# SalesSync AI - Final Fixed Deployment Script
# This script handles Docker build context issues and deploys the complete application
...
required_dirs=("frontend/public" "frontend/src" "backend/src")
...
required_files=("frontend/public/index.html" "frontend/package.json" "backend/package.json")
...
docker rmi $(docker images -q --filter "reference=salessyncai*") 2>/dev/null || true
...
log "Verifying Docker build contexts..."
info "Frontend context files:"
ls -la frontend/ | head -10

info "Backend context files:"
ls -la backend/ | head -10

# Build services individually with detailed output
log "Building backend service..."
if ! $DOCKER_COMPOSE build --no-cache backend; then
    error "Backend build failed!"
    info "Backend Dockerfile content:"
    cat backend/Dockerfile
    exit 1
fi

log "Building frontend service..."
if ! $DOCKER_COMPOSE build --no-cache frontend; then
    error "Frontend build failed!"
    info "Frontend Dockerfile content:"
    cat frontend/Dockerfile.prod
    info "Frontend public directory:"
