...
# SalesSync Ultra-Simple Deployment Script V9
# Enhanced PostgreSQL authentication and robust database setup
...
echo -e "${BLUE}🚀 SALESSYNC ULTRA-SIMPLE DEPLOYMENT V9${NC}"
echo -e "${BLUE}Enhanced PostgreSQL authentication and robust database setup${NC}"
echo ""
...
for i in {1..60}; do
    ...
    if [ $i -eq 60 ]; then
        echo -e "${RED}❌ PostgreSQL failed to start properly after 60 attempts${NC}"
        systemctl status postgresql --no-pager -l
        exit 1
    fi
    echo -e "${YELLOW}⏳ PostgreSQL not ready yet, waiting... (attempt $i/60)${NC}"
    sleep 2
done
...
echo -e "${YELLOW}🔄 Restarting PostgreSQL with new authentication...${NC}"
...
for i in {1..30}; do
    ...
    if [ $i -eq 30 ]; then
        echo -e "${RED}❌ PostgreSQL failed to restart properly${NC}"
        systemctl status postgresql --no-pager -l
        exit 1
    fi
    sleep 2
...
# Create user with explicit error checking
echo -e "${YELLOW}👤 Creating database user...${NC}"
if ! sudo -u postgres psql -c "CREATE USER salessync_user WITH PASSWORD 'salessync_password';" > /dev/null 2>&1; then
...
if ! sudo -u postgres psql -c "CREATE DATABASE salessync OWNER salessync_user;" > /dev/null 2>&1; then
...
if ! sudo -u postgres psql -c "GRANT ALL PRIVILEGES ON DATABASE salessync TO salessync_user;" > /dev/null 2>&1; then
...
echo -e "${GREEN}   ✅ PostgreSQL Database (with authentication configured)${NC}"
