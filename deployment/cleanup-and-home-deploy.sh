...
# SalesSyncAI Complete Cleanup and Home Directory Deployment Script
# This script cleans up old installations in /opt and /root and deploys to home directory
# Designed to avoid permission issues by using home directory installation
...
section "🧹 CLEANUP PHASE: Removing Old Installations"
...
# Clean up old databases (optional - commented out for safety)
# log "🧹 Cleaning up old databases..."
# $SUDO -u postgres psql -c "DROP DATABASE IF EXISTS salessync;" 2>/dev/null || true
# $SUDO -u postgres psql -c "DROP DATABASE IF EXISTS salessync_prod;" 2>/dev/null || true
# $SUDO -u postgres psql -c "DROP USER IF EXISTS salessync_user;" 2>/dev/null || true
# $SUDO -u postgres psql -c "DROP USER IF EXISTS salessync_prod;" 2>/dev/null || true

section "🏠 DEPLOYMENT PHASE: Installing to Home Directory"
...
Security:
- Firewall configured (UFW) if available
- Database password protected
- Redis password protected
- Security headers configured in Nginx

Installation Benefits:
- No permission issues (installed in home directory)
...
log ""
log "🎉 SalesSyncAI has been successfully deployed to your home directory!"
log ""
...
log "  - Stop app: pm2 stop salessync-backend"
log ""
...
log ""
log "🚀 Deployment completed! Your SalesSyncAI application should now be accessible."
log "📖 Check the deployment-info.txt file for detailed configuration information."